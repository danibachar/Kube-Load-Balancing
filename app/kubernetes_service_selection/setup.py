...
# Package meta-data.
NAME = "Kubernetes-Service-Selection"
...
if not VERSION:
    with open(
        os.path.join(here, "src", "kubernetes_service_selection", "__version__.py")
    ) as f:
        exec(f.read(), about)
else:
...
setup(
    ...
    scripts=["bin/run-kubernetes_service_selection-prod"],
    entry_points={
        "console_scripts": [
            "run-kubernetes_service_selection-dev=kubernetes_service_selection.dev_cli:main"
        ]
    },
)
